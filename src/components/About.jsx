...
useEffect(() => {
  ...
  return (
    <section id='about' ref={(ref) => setSectionRef(1, ref)} className='about flex justify-center items-center flex-col  bg-rgba(15, 10, 10, 0.973) pb-24 text-white '>
        <h2 className='text-[3rem] mb-8 text-center top-[80px] font-bold relative'>About <span className='text-[#0bdfee]'>Me</span><span className='animated scroll' style={{ '--1': '1' }}></span></h2>
        <div className='relative w-[25rem] h-[25rem] rounded-[50%] flex justify-center items-center'>
           <img className='w-[35%] rounded-[50%] border-[.2rem] border-[#12dae0]' src={MenorAB} alt="Menor" />
           <span className='circle-spin'></span>
           <span className='animated scroll' style={{ '--1': '2' }}></span>
        </div>
        <div className='text-center'>
            <h3 className='text-[2.6rem] bottom-16 relative inline-block'>Frontend Developer! <span className='animated scroll' style={{ '--1': '3' }}></span></h3>
            <p className= {`text ${showMore ? 'show-more ml-10 mr-10 mb-12 relative' : 'text mt-8 ml-10 mr-10 bottom-16 relative'}`}>
            ...
            <div className='btn-box btn inline-block w-[1000px] h-[50px] bottom-8 relative' onClick={handleClick}>
